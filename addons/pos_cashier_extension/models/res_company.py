...
class ResCompany(models.Model):
    ...
    cashier_console_payment_method_ids = fields.Many2many(
        comodel_name="cashier.payment.method",
        relation="company_cashier_console_payment_method_rel",
        column1="company_id",
        column2="payment_method_id",
        string="Payment methods",
        help="Payment methods available in the Cashier Console.",
    )
    ...
    cashier_console_temp_account_id = fields.Many2one(
        "account.account", string="Default temporary account"
    )
    ...
    cashier_console_send_receipt_whatsapp = fields.Boolean(
        string="Send receipts via WhatsApp"
    )
    ...
    cashier_config_ids = fields.One2many(
        "cashier.config", "company_id", string="Cashier Configurations"
    )
